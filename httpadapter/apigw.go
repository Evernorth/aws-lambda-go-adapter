...
func handleRequestForApigw(httpResponseWriter http.ResponseWriter, httpRequest *http.Request) {
	var err error
	...
	if outputEventIsPointer {
		lambdaResp = outValues[0].Interface().(*events.APIGatewayProxyResponse)
	} else {
		valLambdaResp := outValues[0].Interface().(events.APIGatewayProxyResponse)
